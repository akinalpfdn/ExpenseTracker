...
struct AddMainCategoryDialog: View {
    let onDismiss: () -> Void
    ...
    var body: some View {
        NavigationView {
            ...
            .navigationTitle("add_new_main_category".localized)
            .navigationBarTitleDisplayMode(.inline)
            .navigationBarItems(
                leading: Button("cancel".localized, action: onDismiss),
                trailing: Button("add_button".localized) {
                    onConfirm(categoryName, selectedIconName, selectedColorHex)
                }
    ...
    private var categoryNameSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("category_name_hint".localized)
                .font(.system(size: 14, weight: .medium))
                .foregroundColor(ThemeColors.getTextColor(isDarkTheme: isDarkTheme))

            TextField("category_name_hint".localized, text: $categoryName)
                .textFieldStyle(CustomTextFieldStyle(isDarkTheme: isDarkTheme))

            Text("unique_category_name_note".localized)
                .font(.system(size: 14))
...
// MARK: - Preview
struct AddMainCategoryDialog_Previews: PreviewProvider {
